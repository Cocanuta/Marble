...
public class Import extends StdCommand
{
								...
								Vector commands)
	{
		...
		for(int m=0;m<mobData.size();m++)
		{
			...
			if(circleFormat)
			{
				...
				// ignore the above, marblemud does it better!
				int numAbiles=M.numAbilities();
				M.baseCharStats().getMyRace().startRacing(M,false);
				//if(CMath.isSet(off,0)) // no area killers in marblemud
				//if(CMath.isSet(off,1)) // no circling in marblemud
			...
			for(int mp=0;mp<specialData.size();mp++)
			{
				...
				if(s.startsWith("M "))
				{
					...
					{
						// In marblemud, they all cast bigtime
					}
					else
								...
								Vector commands)
	{
		...
		for(int o=0;o<objectData.size();o++)
		{
			...
			//if((extraFlag&2)==2) marblemud has no hummers
			...
			//if(extraFlag&4096)==4096) marblemud doesn't support rotting cargo

			if(CMath.isSet(extraFlag,14))
				CMLib.flags().setGettable(I,false);

			//if(extraFlag&16384)==16384) marblemud doesn't support rotting cargo
	...
	public static Room applyRoomCodeBits(Room R, Room lastRoom, Map<Room,Room> petShops, long codeBits, boolean circleFormat)
	{
		...
		else
		{
			...
			// if(CMath.isSet(codeBits,23)) No "dirt" in marblemud, so this doesn't matter
					...
					else
					{
						...
						else
						{
							...
							if(M instanceof ShopKeeper)
							{
								...
								if(((I.ID().equals("GenReadable"))
								||(I instanceof com.planet_ink.marble_mud.Items.interfaces.RoomMap))
