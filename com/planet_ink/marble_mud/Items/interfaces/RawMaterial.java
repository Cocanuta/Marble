...
import com.planet_ink.marble_mud.Common.interfaces.CharStats.CODES;
		...
		public CODES(){
			...
			synchronized(this)
			{
				...
				for(int i=0;i<addExtra.length + repExtra.length;i++)
				{
					...
					if(V.size()!=8) {
						Log.errOut("RawMaterial","Bad marblemud.ini material row (requires 8 elements, separated by ,): "+p);
						continue;
					}
					String type="ADD";
					...
					if(replace)
					{
						...
						{
							Log.errOut("RawMaterial","Unknown replaceable resource in marblemud.ini: "+stat);
							continue;
						}
					}
					...
					{
						Log.errOut("RawMaterial","Unknown material code in marblemud.ini: "+matStr);
						continue;
					}
