...
import com.planet_ink.marble_mud.Items.interfaces.RawMaterial.CODES;
		...
		public CODES(){
			...
			synchronized(this)
			{
				...
				for(int i=0;i<addExtra.length+repExtra.length;i++)
				{
					...
					if(V.size()!=6){
						Log.errOut("Wearable","Bad marblemud.ini wear loc row (requires 6 elements, separated by ,): "+p);
						continue;
					}
					String type="ADD";
					...
					if(replace)
					{
						...
						{
							Log.errOut("Wearable","Bad replace worn loc in marblemud.ini file: "+stat);
							continue;
						}
					}
					...
					for(int s=0;s<subLocs.size();s++)
					{
						...
						if(idx>=0) 
							...
							Log.errOut("Wearable","Bad dependency mask in marblemud.ini file: "+((String)subLocs.elementAt(s)).toLowerCase());
					}
