	...
	*/
	...
	public static final void setUpCosts(final String fieldName, final Map<String,ExpertiseLibrary.SkillCostDefinition> map, final Vector<String> fields)
	{
		...
		for(String field : fields)
		{
			...
			if(typeIndex<0)
			{
				Log.errOut("CMProps","Error parsing marblemud.ini field "+fieldName+", value: "+field);
				continue;
			}
			...
			if(costType==null)
			{
				Log.errOut("CMProps","Error parsing marblemud.ini field '"+fieldName+"', invalid type: "+type);
			...
			if(!CMath.isMathExpression(formula, doubleChecker))
			{
				...
				if(skillIndex<0)
				{
					Log.errOut("CMProps","Error parsing marblemud.ini field "+fieldName+", invalid formula: "+formula);
					continue;
				}
			...
			{
				Log.errOut("CMProps","Error parsing marblemud.ini '"+fieldName+"' has duplicate key:"+((keyField.length()==0)?"<EMPTY>":keyField));
				continue;
			}
	...
	public static final String getDefaultMXPImage(final Object O)
	{
		...
		if(O instanceof com.planet_ink.marble_mud.Items.interfaces.RoomMap)
